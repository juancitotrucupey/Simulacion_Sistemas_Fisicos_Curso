...
void Colisionador::CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2,vector3D & dcontactoBall,double & hold,double dt){
  vector3D r21,F2,Fn,Ft,Vc,Vcn,Vct,n,t,t1; 
...
int main(void){
  ...
  //Fronteras
  //Suelo o paño
  Grano[N].Inicie(Lx/2,Ly/2,(-1)*Radiote,0,0,0,0,0,0,Radiote,1);
    //Banda Oeste
  Grano[N+1].Inicie(-Radiote,Ly/2,BallRadious/2,0,0,0,0,0,0,Radiote,1);
    //Banda Este
  Grano[N+2].Inicie(Lx+Radiote,Ly/2,BallRadious/2,0,0,0,0,0,0,Radiote,1);
    //Banda Sur
  Grano[N+3].Inicie(Lx/2,(-1)*Radiote,BallRadious/2,0,0,0,0,0,0,Radiote,1);
    //Banda Norte
  Grano[N+4].Inicie(Lx/2,Ly+Radiote,BallRadious/2,0,0,0,0,0,0,Radiote,1);
...
//Grano[0].Inicie(double x0,double y0,double z0,double Vx0,double Vy0,double Vz0,double omegax0,double omegay0,double omegaz0, double R0, double m0);  
  ...
  Grano[0].Inicie(0.1,0.2,BallRadious,0,20,0,1,1,100,BallRadious,BallMass);

  Grano[1].Inicie(0.4,0.2,BallRadious,-10,-2,0,1,1,1,BallRadious,BallMass);
  
  Grano[2].Inicie(0.4,0.4,BallRadious,-8,-5,0,1,1,1,BallRadious,BallMass); 
