...
class Colisionador{
private:
  vector3D  dcontactoBall[N][N+5], Vt[N][N+5];
  double hold[N][N+5];
 public:
  void Inicie(void);
  void CalculeTodasLasFuerzas(Cuerpo * Grano,double dt);
  void CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2,vector3D &dcontactoBall,vector3D &Vt,double &hold,double dt);
...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * Grano,double dt){
  ...
  for(i=0;i<N;i++){
       for(j=i+1;j<N;j++){
		 	 CalculeFuerzaEntre(Grano[i],Grano[j],dcontactoBall[i][j],Vt[i][j],hold[i][j],dt);
		        }
  }
...
void Colisionador::CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2,vector3D & dcontactoBall,vector3D & Vt,double & hold,double dt){
