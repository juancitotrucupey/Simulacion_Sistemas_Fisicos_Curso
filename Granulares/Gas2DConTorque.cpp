...
private:
  ...
  double m,R,I,Theta;
public:
  void Inicie(double x0,double y0,double z0,double Vx0,double Vy0,double Vz0,double Wx0, double Wy0,double Wz0,double Theta0,double m0,double R0);
  void BorreFuerza(void);
  void AdicioneFuerza(vector3D F0);
  void AdicioneTorque(vector3D T0);
  void AgregeFuerzaGravedad();
...
void Cuerpo::Inicie(double x0,double y0,double z0,double Vx0,double Vy0,double Vz0,double Wx0, double Wy0,double Wz0,double Theta0,double m0,double R0);
r.cargue(x0,y0,z0);   V.cargue(Vx0,Vy0,Vz0); Theta.cargue(Wz0,Wy0,Wz0); Theta=Theta0;   m=m0;  R=R0; I=2.0*m*R*R/5;
}
void Cuerpo::BorreFuerzaTorque(void){
  F.cargue(0,0,0); T.cargue(0,0,0);
}
...
void Cuerpo::AgregueFuerzaGravedad(vector3D G0){
  vector3D peso;
...
void Cuerpo::Dibujese(void){
  cout<<" , "<<r.x()<<"+"<<R<<"*cos(t),"<<r.y()<<"+"<<R<<"*sin(t) , "<<r.x()<<"+"<<R*cos(theta)/7<<"*t,"<<r.y()<<"+"<<R*sin(theta)/7<<"*t";
}
...
int main(void){
  ...
  T=sqrt(Lx*2/g)/VEL0;  tmax=3*T;
  
  InicieAnimacion();


  //(double x0,double y0,double z0,double Vx0,double Vy0,double Vz0,double Wx0, double Wy0,double Wz0,double Theta0,double m0,double R0);
   //PAREDES
  //Pared izquierda
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      Alpha=2*M_PI*ran2.r();
      Grano[iy*Nx+ix].Inicie(dx*(ix+1),dy*(iy+1), 0,VEL0*cos(Alpha),VEL0*sin(Alpha),0,0,0,1,0,m0,R0);
    }
