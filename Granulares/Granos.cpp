...
public:
  void Inicie(void);
  void CalculeTodasLasFuerzas(Cuerpo * Grano,double dt,bool test);
  void CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2,vector3D &dcontacto,double &hold,double dt,bool test);
  double GetEp(void);
};
...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * Grano,double dt,bool test){
  int i,j;
  ...
  for(i=0;i<N;i++)    
    for(j=i+1;j<N+4;j++)
      CalculeFuerzaEntre(Grano[i],Grano[j],dcontacto[i][j],hold[i][j],dt,test);
}
void Colisionador::CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2,vector3D &dcontacto,double &hold,double dt,bool test){
...
int main(void){
  ...
  double rho0=1,m0,R00,R0,omega0;
  double T,VEL0=1,Alpha,dx,dy;
  bool test=false;
  ...
  //PAREDES
  //Pared izquierda
    Grano[N].Inicie(-10000  ,Ly/2   , 0,  0,  0,  0,      0,      0,       0,    0,10,10000); 
  //Pared derecha
  ...
  for(t=0,tdibujo=0;t<tmax;t+=Deltat,tdibujo+=Deltat){
    ...
    Hertz.CalculeTodasLasFuerzas(Grano,Deltat,test); for(i=0;i<N;i++) Grano[i].Mueva_V(Deltat,uno_m2LAMBDA_S2);
    for(i=0;i<N;i++) Grano[i].Mueva_r(Deltat,XI);
    Hertz.CalculeTodasLasFuerzas(Grano,Deltat,test); for(i=0;i<N;i++) Grano[i].Mueva_V(Deltat,LAMBDA); 
