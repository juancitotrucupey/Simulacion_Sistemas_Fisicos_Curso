...
const int N=3; //número de Planetas
//............................
class Cuerpo;
class Colisionador;
//............................
...
public:
  ...
  void Dibujese(void);
  double GetCosB(void){return r.x()/norma(r);}; //Funcion inline (macro)
  double GetSinB(void){return r.y()/norma(r);}; //Funcion inline (macro)
...
int main(void){
  ...
  double x0, x1, V0, V1;
  double Beta=M_PI/3;
  vector3D Troyano, TroyanoV;
  Troyano.cargue(cos(Beta)*r+x0,sin(Beta)*r,0);
    int i;
  double cosAlpha, sinAlpha;
  cosAlpha=Troyano.x()/norma(Troyano); sinAlpha=Troyano.y()/norma(Troyano);
  M=m0+m1; Omega=sqrt(G*M/(r*r*r)); T=2*M_PI/Omega;
  x0=-r*m1/M; x1=r*m0/M; V0=Omega*x0; V1=Omega*x1;
    
  tmax=20.5*T;
  TroyanoV.cargue((-1)*Omega*sinAlpha*x1,Omega*cosAlpha*x1,0);
  ...
  Planeta[2].Inicie(Troyano.x()+Deltax,Troyano.y()+Deltay,0,TroyanoV.x(),TroyanoV.y(),0,0.005,5);//Troyano
  
  
  //------------(x0,y0,Vx0,Vy0,m0   ,R0);
	for(t=0,tdibujo=0;t<tmax;t+=dt,tdibujo+=dt){
	  //  cout<<Planeta[1].GetCosB()<<" "<<Planeta[1].GetSinB()<<endl;
	  	  	  cout<<Planeta[1].GetCosB()* Planeta[2].Getx()+Planeta[1].GetSinB()*Planeta[2].Gety()<<" "<<(-1)*Planeta[1].GetSinB()* Planeta[2].Getx()+Planeta[1].GetCosB()*Planeta[2].Gety()<<endl;
		//Animacion
