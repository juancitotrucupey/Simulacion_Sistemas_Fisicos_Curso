...
const double AngIncidente = 60.0*M_PI/180.0;
...
double LatticeBoltzmann::Ccelda(int ix, int iy){
  //  return 0.5;
    double C;
    C=(0.25/2)*((-1)*tanh(ix-20+(iy-200)*tan(AngIncidente)))+0.375;
  return C;
...
void LatticeBoltzmann::Imprimase(char * NombreArchivo,int t){
  ...
  for(int ix=0;ix<Lx/2;ix++){
    for(int iy=0;iy<Ly;iy++)
      //  MiArchivo<<ix<<" "<<iy<<" "<<(0.5/2)*((-1)*tanh(ix-100+(iy-100)*tan(AngIncidente)))+0.75<<endl;
...
void LatticeBoltzmann::ImprimaUnaLinea(char * NombreArchivo,int t){
  ...
  int iy=Ly/2;
  for(int ix=0;ix<Lx/2;ix++)
    //   MiArchivo<<ix<<" "<<(0.5/2)*((-1)*tanh(ix-300+(iy-100)*tan(AngIncidente)))+0.75<<endl;
